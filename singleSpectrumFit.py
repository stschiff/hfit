...
bool no_mcmc;
size_t mcmc_steps = 3500;
size_t mcmc_burnin = 1500;
string spectrumFilename;
string mcmcStepFilename = "/dev/null";
...
void readCommandlineParams(string[] args) {
  getopt(args, std.getopt.config.passThrough, "model|m", &model, "maxSteps", &maxSteps, "mu", &mu, "V", &V, "t", &t, "no_mcmc", &no_mcmc, "spectrumFile", &spectrumFilename, "mcmcStepFile", &mcmcStepFilename);
...
int[] readSpectrum() {
  ...
  auto spectrum = inputFile.byLine.map!(line => line.split()[1].to!int())().array;
...
void writeParamHeader(File file) {
  file.writeln("mu\tV\tt\ts\tgamma\tc\tcw\tscore");
}
...
void writeParams(File file, SingleSpectrumScore scoreFunc, double[] x) {
  ...
  file.writefln("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s", p["mu"], p["V"], p["t"], p["s"], p["gamma"], p["c"], p["cw"], score);
}
